...
window.run = function(worker) {
    ...
    server.on('request', function(req, res) {
        ...
        req.on('end', function() {
            parseURI(req.url).then(function(uriData) {
                getImage('https://cdn.google.com' + uriData.origUri).then(function(imageData) {
                    sharp(Buffer.from(imageData.data, 'binary')).resize(uriData.resizeWidth, uriData.resizeHeight).toBuffer().then(function(resizedImage) {
                        var imagemin = require('imagemin');
                        var imageminPngquant = require('imagemin-pngquant');
                        imagemin.buffer(resizedImage, {
                            plugins: [
                                require('imagemin-mozjpeg')(), imageminPngquant(),
                            ]
                            ...
                            console.log('Cannot optimize image: ' + req.url) console.log(err) res.writeHead(200, {
                                'Content-Type': imageData.contentType,
...
function parseURI(uri) {
    return new Promise(function(resolve, reject) {
        ...
        data['uri'] = uri; data['resizeWidth'] = +parseInt(resizeParams.split(/x|X/)[0]) || null;
